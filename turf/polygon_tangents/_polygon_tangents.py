...
def polygon_tangents(
    ...
    """
    start_point = normalize_feature(start_point, ("Point"))
    polygon = normalize_feature(polygon, ("Polygon", "MultiPolygon"))

    point_coord = get_coords_from_features(start_point)
...
def normalize_feature(
    geojson: GeoJson, geojson_types: str
) -> Union[Feature, FeatureCollection]:
    """ Normalizes the input feature to a Feature or FeatureCollection
    ...
    feature_flag = False
    try:
        current_type = geojson.get("type")
    except (NameError, AttributeError):
        raise InvalidInput(error_code_messages["InvalidGeometry"](all_geometry_types))

    if current_type in ["Feature", "FeatureCollection"]:
        feature_flag = True
        current_type = geojson.get("geometry").get("type")

    if current_type in geojson_types:

        if not feature_flag:
            geojson = feature(geojson)

    else:
        raise InvalidInput(error_code_messages["InvalidGeometry"](geojson_types))
